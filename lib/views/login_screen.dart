...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void login() async {
    sessionModel = await apiManager.loginUser(
        nameController.text.toString(), passwordController.text.toString());
    if (sessionModel.accessToken == "Hata") {
      key.currentState!.showSnackBar(
          SnackBar(content: Text("${sessionModel.refreshToken.toString()}")));
    } else {
      Navigator.of(context).pop();
      Navigator.push(context,
          MaterialPageRoute(builder: (context) => HomeScreen(sessionModel)));
    }
  }
          ...
          if (!value.isEmpty)
            {
              ...
              Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => HomeScreen(sessionModel))),
            },
        });
  }
}
